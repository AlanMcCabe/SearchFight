...
/**
 * Class to execute all queries and compare the results. Execution of queries
 * is delegated to an instance of the @QueryExecutor class.
 * 
...
public class QueryComparator {
   ...
   * 
   * Queries are executed one by one on all search engines with results returned as a 
   * Map of Long results keyed on SearchEngines instances. Each map of results is compared
   * to the current top query for each search engine as well as an overall "winning" query
   * determined by the total number of results across all search engines.
   * 
   * The results of each query are logged as they are received, with the winners for
   * each search engine as well as the overall winning query logged once all queries
   * have been executed and compared.
   * 
  ...
  /**
   * Compare the result of a query with the current top query result for a certain
   * search engine. If the result is greater than the current winning query, replace
