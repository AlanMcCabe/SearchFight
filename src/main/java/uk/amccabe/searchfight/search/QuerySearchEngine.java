 ...
 * 
 * The query is executed using Jsoup impersonating a web browser in order to have the full
 * HTML returned. Once this has been received we can extract the number of results from
 * a known area in the Document as specified in @searchEngines.json and return it as a
 * String.
 * 
...
public class QuerySearchEngine {
  ...
  public String executeQuery(String queryString, SearchEngine engine) {
    try {
      ...
      if (resultStats == null) {
        throw new RuntimeException("Unable to find result stats.");
      }
